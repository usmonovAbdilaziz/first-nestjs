...
export class ItemObjectsService {
  ...
  async create(createItemObjectDto: CreateItemObjectDto) {
    try {
      const { category_id, name, sub_category_id } = createItemObjectDto;
      ...
      const subCategory =
        await this.subCategoryService.findOne(sub_category_id);
      ...
      const newObject = this.itemObjRepo.create({ ...createItemObjectDto });
  ...
  async findAll() {
    try {
      const items = await this.itemObjRepo.find({
        relations: ['category'],
        order: { name: 'ASC' },
      });
